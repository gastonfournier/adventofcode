...
public class Day13 implements Solution<Integer, Integer> {
  ...
  List<Tuple<PacketData, PacketData>> processInput(List<String> input) {
    ...
    while (it.hasNext()) {
      ...
      if (!current.isBlank()) {
        packets.add(new Tuple<>(parsePacket(current), parsePacket(it.next())));
      }
    }
    return packets;
  }
  ...
  class PacketData implements Comparable<PacketData> {
    ...
    public String toString() {
      return "["
          + data.stream()
              .map(p -> p.isLeft() ? p.getLeft().toString() : p.get().toString())
    ...
    public int compareTo(PacketData other) {
      ...
      return Integer.compare(
          data.size(),
          other.data.size()); // if the first list run out of items first, they're in order
    }
  }
}
