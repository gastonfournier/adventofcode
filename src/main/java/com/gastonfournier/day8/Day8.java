...
public class Day8 implements Solution<Integer, Integer> {
    ...
    public Integer part1(List<String> input) {
        ...
        for (int i = 1; i < matrix.length - 1; i++) {
            for (int j = 1; j < matrix[i].length - 1; j++) {
                if (
                        isVisible(matrix, matrix[i][j], upOrLeft.apply(i), j, upOrLeft, noMove) ||
    ...
    private boolean isVisible(int[][] matrix,
                              int checkedHeight,
                              int x, int y,
        ...
        if (x >= 0 && x < matrix.length && y >= 0 && y < matrix[x].length) {
            return checkedHeight > matrix[x][y] &&
                    isVisible(matrix, checkedHeight, lateralMove.apply(x), verticalMove.apply(y), lateralMove, verticalMove);
        } else {
            return true; // I made it out of the matrix so this one is visible
        }
    }

    private int scenicScore(int[][] matrix,
                            int checkedHeight,
                            int x, int y,
                            IntFunction<Integer> lateralMove,
                            IntFunction<Integer> verticalMove) {
        ...
        if (x >= 0 && x < matrix.length && y >= 0 && y < matrix[x].length) {
            if (checkedHeight > matrix[x][y]) {
                return 1 + scenicScore(matrix, checkedHeight, lateralMove.apply(x), verticalMove.apply(y), lateralMove, verticalMove);
            } else {
                return 1;
            }
        } else {
        ...
        for (int i = 1; i < matrix.length - 1; i++) {
            for (int j = 1; j < matrix[i].length - 1; j++) {
                int scenicScore =
                        ...
                        scenicScore(matrix, matrix[i][j], bottomOrRight.apply(i), j, bottomOrRight, noMove) *
                        scenicScore(matrix, matrix[i][j], i, bottomOrRight.apply(j), noMove, bottomOrRight);
