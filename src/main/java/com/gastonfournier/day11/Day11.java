...
public class Day11 implements Solution<BigInteger, BigInteger> {
  ...
  private static Pattern decisionPattern =
      Pattern.compile("\s*If (?:true|false): throw to monkey (\\d+)");
  private static Pattern divisibleByPattern = Pattern.compile("\s*Test: divisible by (\\d+)");

  private static Pattern binaryOperationPattern =
      Pattern.compile("\s*Operation: new = (\\d+|old)\s(\\+|\\*)\s(\\d+|old)");
  ...
  private BigInteger playWithMonkeys(
      List<String> input, int rounds, Function<BigInteger, BigInteger> reliefFn) {
    ...
    for (int i = 0; i < rounds; i++) {
      ...
      // System.out.println("Items after round " + (i+1) + ": "+ monkeys.stream().map(m ->
      // m.items.size()).reduce(Math::addExact).get());
    }

    List<Integer> sorted = inspections.values().stream().sorted().toList();
    System.out.println(
        inspections.entrySet().stream().map(e -> e.getKey() + " -> " + e.getValue()).toList());
    return BigInteger.valueOf(sorted.get(sorted.size() - 1))
        .multiply(BigInteger.valueOf(sorted.get(sorted.size() - 2)));
  }
    ...
    while (it.hasNext()) {
      ...
      if (line.trim().startsWith("Monkey")) {
        ...
        Function<BigInteger, Boolean> test =
            (BigInteger n) -> n.mod(BigInteger.valueOf(divisor)).equals(BigInteger.ZERO);
  ...
  private List<Item> parseStartingItems(String line) {
    return Arrays.stream(
            line.replaceFirst("\s*Starting items: ", "").replaceAll("\s*,\s*", ",").split(","))
        .map(BigInteger::new)
        .map(Item::new)
        .toList();
  }
  ...
  class Monkey {
    ...
    Monkey(
        ...
        Function<BigInteger, Boolean> test,
        int destinationWhenTrue,
        int destinationWhenFalse) {
      this.name = name;
