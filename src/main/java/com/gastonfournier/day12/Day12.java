...
public class Day12 implements Solution<Integer, Integer> {
    ...
    private int dijkstra(Tuple<Integer, Integer> origin, Tuple<Integer, Integer> destination, char[][] matrix) {
        ...
        var shortestDistanceFirst = new PriorityQueue<Integer>((o1, o2) ->
    ...
    private int dfs(Tuple<Integer, Integer> origin, Tuple<Integer, Integer> destination, char[][] matrix) {
        int height = matrix.length;
        int width = matrix[0].length;
    ...
    private Optional<Integer> tryPath(char[][] matrix, char currentHeight, int x, int y) {
        int width = matrix[0].length;
        if (x >= 0 && x < matrix.length && y >= 0 && y < width &&
                matrix[x][y] <= currentHeight + 1) {
